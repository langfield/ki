...
def html_to_screen(html: str) -> str:
    ...
    # This sentinel attribute is added to the top-level tag by
    # `markdown_to_html()` (a part of the `apy` library), so it is only present
    # when a note has been added in a `push()` whose `markdown` field is set to
    # `True`. But `get_header_lines()` only sets this field to `True` during a
    # `clone()` call if the sentinel is already there. So this will only happen
    # if the user sets it to `True` while editing a note file generated by
    # `ki`.
    # TODO: Copy remaining `apy` methods into ki, and change the sentinel text
    # to something more meaningful. Also, consider changing the field name
    # `markdown` to something more indicative of its use, like `ki-generated`.
