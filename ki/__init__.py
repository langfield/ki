...
collection, and the ``.anki2`` collection file as a remote. All operations like
pulling updates to the collection into `ki` and pushing updates from `ki` into
Anki are handled by git under the hood. This means merge conflicts can be
handled in the usual, familiar way, and additional remotes (e.g. a
human-readable backup of a collection on github) can be added easily.  Users
are free to pick the editor of their choice, perform batch editing with command
line tools like `awk` or `sed`, and even add CI actions. In general, the
purpose of `ki` is to allow users to work on large, complex Anki decks in
