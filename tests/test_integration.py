...
def test_clone_writes_plaintext_posix_symlinks_on_windows(tmp_path, mocker: MockerFixture):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        winlinks = [
            Path(SYMLINKS.repodir) / "Default" / "B" / "sample_cloze-ol.md",
            Path(SYMLINKS.repodir) / "Default" / "B" / "sample_cloze-ol_1.md",
            Path(SYMLINKS.repodir) / "Default" / "C" / "sample_cloze-ol.md",
            Path(SYMLINKS.repodir) / "Default" / "C" / "sample_cloze-ol_1.md",
            Path(SYMLINKS.repodir) / "Default" / "C" / "sample_cloze-ol_2.md",
            Path(SYMLINKS.repodir) / "Default" / "C" / "sample_cloze-ol_3.md",
            Path(SYMLINKS.repodir) / "Default" / "C" / "sample_cloze-ol_4.md",
                ]

        repo = git.Repo(SYMLINKS.repodir)
        for entry in repo.commit().tree.traverse():
            if isinstance(entry, git.Blob):
                path = entry.path
                file_mode = entry.file_mode
                logger.debug(f"path : {type(path)} = {path}")
                logger.debug(f"file_mode : {type(file_mode)} = {file_mode}")
        g = git.Git(SYMLINKS.repodir)
        for path in winlinks:
            out = g.ls_files(["-s", str(path)])
            logger.debug(out)
